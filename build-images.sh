...
function build_and_push_image () {
  ...
  docker buildx build -t $DOCKER_REPO/$BLOCK_NAME:$BALENA_MACHINE_NAME --progress=plain --load --platform $DOCKER_ARCH --file Dockerfile.$BALENA_MACHINE_NAME .
...
function create_and_push_manifest() {
  local DOCKER_REPO=$1
  docker manifest create --insecure $DOCKER_REPO/$BLOCK_NAME:latest \
  ...
  docker manifest push --insecure --purge $DOCKER_REPO/$BLOCK_NAME:latest
